...
async function cargarSecuencial() {
    ...
    try {
        ...
        mostrarLog(`Secuencial completado en ${formatearTiempo(total)}`, 'info');
        mostrarComparativa();
    } catch (error) {
...
async function simularError() {
    ...
    try {
        ...
        mostrarLogError('El error fue manejado correctamente con try/catch', 'success');
    }
}
...
document.getElementById('btn-reintentos').addEventListener('click', () => {
    fetchConReintentos('Recurso', 3).catch(() => {
        mostrarLogError('Proceso de reintentos completado (Fallido)', 'info');
    });
});